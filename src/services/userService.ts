...
export class UserService implements IUserService {
  async create(userData: Partial<IUser>, currUser?: IUser): Promise<IUser> {
    ...
    if (userData.role === UserRole.Admin)
      throw new ApiError(ApiErrors.InsufficientPermissions);
    if (userData?.role === UserRole.School && currUser?.role !== UserRole.Admin)
      throw new ApiError(ApiErrors.InsufficientPermissions);
  ...
  async bulkStudentInsertOrUpdate(
    students: Array<any>,
    currUser?: IUser
  ): Promise<any> {
    let totalAdded = 0;
    ...
    const bulkOps = students.map((student, idx) => ({
      updateOne: {
        ...
        update: {
          $set: {
            ...student,
            username: idx + 'STU' + tempNumber,
            schoolId: schoolId,
            role: 'student',
            password:
              '$2b$10$Jo4ApxQMs/4cawgtHhbXMOrSNfhB3.6SAaosPaF8qcgMaqDS3HCsW',
          },
        },
        upsert: true,
      },
