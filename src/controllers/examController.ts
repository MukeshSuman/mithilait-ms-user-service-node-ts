...
export class ExamController extends Controller {
  ...
  public async getById(
    ...
    const result =
      await this.examService.getSingleExamWithStudentsReportAndPagination(
        id,
        currUser
      );
  ...
  public async submitExam(
    ...
    return new ApiResponse(200, true, 'Exam submitted successfully', {
      ...examResults,
      report: result,
      file: fileResult,
    });
  ...
  public async getAll(
    ...
    const result =
      await this.examService.getExamWithStudentsReportAndPagination(
        options,
        currUser
      );
    return new ApiResponse(200, true, 'Exam retrieved successfully', result);
  ...
  _prepareData = (data: Array<any>) => {
    ...
    data?.map((exam: any) => {
      exam.students?.map((student: any) => {
        ...
        const newObject = transformObject(
          { ...overrideObject, ...stu },
          {
            selectKeys: [],
            renames: {
              ...
              exam_type: 'Type',
              exam_topic: 'Topic',
              report_result_accuracyScore: 'AccuracyScore',
              report_result_completenessScore: 'CompletenessScore',
              report_result_fluencyScore: 'FluencyScore',
              report_result_pronunciationScore: 'PronunciationScore',
              report_result_prosodyScore: 'ProsodyScore',
            },
            newKeys: {
              Name: 'firstName,lastName',
              StudentClass: 'class,section',
              ExamFor: 'exam_class,exam_section',
            },
          }
        );
        const reorderKeys = reorderObjectKeys(newObject, [
          'Name',
          'StudentClass',
          'Title',
          'Type',
