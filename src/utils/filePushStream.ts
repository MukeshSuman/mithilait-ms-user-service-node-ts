...
function readInt32(fd: number): number {
  ...
  if (bytesRead !== 4) {
    throw new Error(
      `Error reading 32-bit integer from .wav file header. Expected 4 bytes, got ${bytesRead}`
    );
  }
...
function readUInt16(fd: number): number {
  ...
  if (bytesRead !== 2) {
    throw new Error(
      `Error reading 16-bit unsigned integer from .wav file header. Expected 2 bytes, got ${bytesRead}`
    );
  }
...
function readUInt32(fd: number): number {
  ...
  if (bytesRead !== 4) {
    throw new Error(
      `Error reading unsigned 32-bit integer from .wav file header. Expected 4 bytes, got ${bytesRead}`
    );
  }
...
function readString(fd: number, length: number): string {
  ...
  if (bytesRead !== length) {
    throw new Error(
      `Error reading string from .wav file header. Expected ${length} bytes, got ${bytesRead}`
    );
  }
...
export const readWavFileHeader = (audioFileName: string): WavFileHeader => {
  ...
  if (formatSize > 16) {
    throw new Error(
      `Error reading .wav file header. Expected format size 16 bytes. Actual size: ${formatSize}`
    );
  }
  // Format tag
